...
function changeRgb(channel, value) {
  switch (channel) {
    case "red":
      rgb[0] = value; //updating the value based on the channel index in the rgb array. 0 for red, 1 for green, and 2 for blue. The value should be between 0 and 255.
      break;
    case "green":
      rgb[1] = value;
      break;
...
// global rgb array element [0, 1, 2]
...
function writeCSS(newRgb) {
  var newColor = "rgb(";
  var i = 0; //using this as the initializer for the while loop
  while (i < newRgb.length) {
    //adding the newColor value to the newRgb array. Should loop through each element in the array to complete the rgb(value, value, value)
    newColor += newRgb[i];
    //adding a comma and space between each value
    newColor += ", ";
    //incrementing the value of i/the index of the newRgb array
    i++;
  }
  //including all the elements except the last comma and space with slice
  newColor = newColor.slice(0, -2);
  //adding the )" to the end of the string
  newColor += ")";
  //returning the newColor to the event handler
  return newColor;
}
